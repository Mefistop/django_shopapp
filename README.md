...
```bash
git clone https://github.com/ykozhanov/Django-OnlineShopping.git
```
...
docker compose up
```
Докер автоматически создаст суперпользователя с email и паролем указанными в .env
...
можно писать на почту `bajjkov.petr@rambler.ru`